...
p = 1
trials = {5: 10000, 6: 10000}

indices = {'banzhaf': lambda weights, q: banzhaf(weights, q, True), 'shapley': shapley}#, 'generalized_banzhaf': lambda weights, q: banzhaf(weights, q, q, True)}
...
for n in trials.keys():
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig(f'plots/veto_distortion/veto_distortion_n{n}.png')
    plt.close()
